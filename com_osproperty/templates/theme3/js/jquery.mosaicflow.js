...
(function(factory) {  // Try to register as an anonymous AMD module
	...
	Mosaicflow.prototype = {
		...
		ensureColumns: function() {
			...
			if (calculatedCnt > createdCnt) {
				...
				for (var columnIdx = 0; columnIdx < neededCnt; columnIdx++) {
					var column = $('<div>', {
						'class': this.options.columnClass + ' column' + columnIdx
					});
		...
		fillColumns: function() {
			...
			*/
			if(itemsCnt >= 5){
				var itemforCol0 = new Array(0,2);
				var itemforCol1 = new Array(1,3,4);
			}else if(itemsCnt == 4){
				var itemforCol0 = new Array(0,2);
				var itemforCol1 = new Array(1,3);
				this.columns.eq(0).css('width', '50%');
				this.columns.eq(1).css('width', '50%');
			}else if(itemsCnt == 3){
				var itemforCol0 = new Array('0');
				var itemforCol1 = new Array(1,2);
			}else if(itemsCnt == 2){
				var itemforCol0 = new Array('0');
				var itemforCol1 = new Array('1');
				this.columns.eq(0).css('width', '60%');
				this.columns.eq(1).css('width', '40%');
			}
			//for column 0
			var column0 = this.columns.eq(0);
			this.columnsHeights[0] = 0;
			for(var i = 0; i < itemforCol0.length;i++){
				var item = this.items.eq(itemforCol0[i]);
				var height = 0;
				column0.append(item);
				...
				this.itemsHeights[item.attr('id')] = height;
				this.columnsHeights[0] += height;
			}

			//for column 1
			var column1 = this.columns.eq(1);
			this.columnsHeights[1] = 0;
			for(var i = 0; i < itemforCol1.length;i++){
				var item = this.items.eq(itemforCol1[i]);
				var height = 0;
				column1.append(item);
				...
				this.itemsHeights[item.attr('id')] = height;
				this.columnsHeights[1] += height;
			}

			var width = jQuery(window).width();
			if(width < 600){
				column0.css('width', '100%');
				column1.css('width', '0%');
			}
			...
			if(itemsCnt == 3){
				var height = jQuery('.gallery_photo1').height();
				jQuery('.mosaicflow').css('max-height',height + 'px');
			}else if(itemsCnt == 2){
				var height = jQuery('.gallery_photo1').height();
				jQuery('.mosaicflow').css('max-height',height + 'px');
				jQuery('#googlemapdiv').css('height',height +'px');
			}else if(itemsCnt == 4){
				var height = jQuery('.gallery_photo1').height();
				jQuery('#googlemapdiv').css('height',height +'px');
			}
