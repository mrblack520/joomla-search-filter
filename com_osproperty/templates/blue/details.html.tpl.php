...
<div class="lightGrad detailsView clearfix">
	<div class="<?php echo $bootstrapHelper->getClassMapping('row-fluid'); ?>">
		...
		<div class="<?php echo $bootstrapHelper->getClassMapping('span'.$span); ?>">
				                ...
				                for($i=0;$i<count($photos);$i++){
				                	if($photos[$i]->image != ""){
				                		if(JPATH_ROOT.'/images/osproperty/properties/'.$row->id.'/medium/'.$photos[$i]->image){
				                			?>
				                			<li class="propertyinfoli"><a class="propertyphotogroup" href="<?php echo JURI::root()?>images/osproperty/properties/<?php echo $row->id;?>/<?php echo $photos[$i]->image?>"><img src="<?php echo JURI::root()?>images/osproperty/properties/<?php echo $row->id;?>/medium/<?php echo $photos[$i]->image?>" alt="<?php echo $photos[$i]->image_desc;?>" title="<?php echo $photos[$i]->image_desc;?>"/></a></li>
				                			<?php
				                		}else{
				                			?>
				                ...
				                for($i=0;$i<count($photos);$i++){
				                	if($photos[$i]->image != ""){
										if(JPATH_ROOT.'/images/osproperty/properties/'.$row->id.'/thumb/'.$photos[$i]->image){
				                			...
				                			?>
											<a data-slide-index="<?php echo $i;?>" href=""><img src="<?php echo JURI::root()?>media/com_osproperty/assets/images/nopropertyphoto.png" /></a>
											<?php
				                		}
				                	}else{
				                		?>
	                        ...
	                        if(file_exists(JPATH_ROOT.DS."components".DS."com_oscalendar".DS."oscalendar.php")){
								if(($configClass['integrate_oscalendar'] == 1) and (in_array($row->pro_type,explode("|",$configClass['show_date_search_in'])))){
                                    ...
                                    if($count > 0) {
                                        ?>
                                        <li>
                                            <span class="btn btn-info width80pc">
                                                <a href="#oscalendardiv" class="colorwhite fontbold nodecoration"><?php echo JText::_('OS_BOOK_NOW')?></a>
                                            </span>
                                        </li>
                                    <?php
                                    }
	                        ...
	                        <div class="details">
	                            <ul>
	                                <li class="propertyinfoli">
	                                	<a href="<?php echo JRoute::_("index.php?option=com_osproperty&task=agent_info&id=".$row->agent_id."&Itemid=".OSPRoute::getAgentItemid($row->agent_id));?>"><?php echo $row->agent_name;?></a>
	                            	</li>
	                            	<?php
	        ...
	        <div class="<?php echo $bootstrapHelper->getClassMapping('row-fluid'); ?> clearfix">
	    ...
	    <div class="<?php echo $bootstrapHelper->getClassMapping('span4'); ?>">
	    	<!-- request -->
			<?php
			HelperOspropertyCommon::requestMoreDetailsTop($row,$itemid);
			?>
	        <!-- end request -->
	    </div>
	    <?php
			...
			for($i=0;$i<count($extra_field_groups);$i++){
				...
				?>
					...
					<div class="<?php echo $bootstrapHelper->getClassMapping('row-fluid'); ?>">
						<?php
						$k = 0;
						for($j=0;$j<count((array)$fields);$j++){
							$field = $fields[$j];
							if($field->field_type != "textarea"){
								...
								if($k == $jump){
									?>
									</div><div class='<?php echo $bootstrapHelper->getClassMapping('row-fluid'); ?>' style='min-height:0px !important;'>
									<?php
									$k = 0;
								}
							}
