...
if (!class_exists('OSPJPagination')){
	class OSPJPagination extends JObject
	{
		...
		function _item_active(&$item)
		{
			...
			}elseif(($item->text == JText::_("OS_END")) || ($item->text == JText::_("OS_START"))){
				$hiddenphone = "hidden-phone";
			}else{
				$hiddenphone = "";
			}
			...
			if($configClass['fix_page_navigation_issue'] == 1)
			{
				$priority_task = array('company_properties','property_advsearch','property_manageallproperties','agent_editprofile','agent_default','category_details','agent_listing','agent_layout','property_type');
				$priority_view = array('lmanageproperties','aeditdetails','lagents','ladvsearch','ltype');
			}
			else
			{
			...
			if((in_array($task,$priority_task) || in_array($view,$priority_view)) && $configClass['fix_page_navigation_issue'] == 1)
			{
				if($item->base>0)
				{
			...
			{
				return "<li class='".$hiddenphone."'><a title=\"".$item->text."\" href=\"".$item->link."\" class=\"pagenav\">".$item->text."</a></li>";
			}
		}
	...
	if (!class_exists('JPaginationClone')){
		class JPaginationClone extends JObject
		{
			...
			function getLimitBox()
			{
				...
				if($mainframe->isClient('administrator'))
				{
					$html = JHTML::_('select.genericlist',  $limits, 'limit', 'class="input-mini form-control form-select" size="1" onchange="submitform();"', 'value', 'text', $selected);
				} 
				else 
			{
					$html = JHTML::_('select.genericlist',  $limits, 'limit', 'class="input-mini form-control form-select" size="1" onchange="document.ftForm1.submit()"', 'value', 'text', $selected);
				}
				return $html;
			}
